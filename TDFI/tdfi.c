...
int main(int argc, char* argv[]){
    ...
    */
    ...
    double *resultado_tdfi=malloc(num_muestras/2 * sizeof(double));
    ...
    //Dividimos mas muestras entre reales e imaginarios
    int iR=0;
    int iI=0;
    ...
    tdfi(reales,imaginarios,resultado_tdfi,num_muestras/2);

    iR=0;

    for(int i=0;i<num_muestras;i++){
        if(i%2==0){
            resultado[i]=resultado_tdfi[iR];
            iR++;
        }else{
            resultado[i]=0.0;
        }
